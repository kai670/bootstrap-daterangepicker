...
!function($) {
	...
	var DateRangePicker = function(element, options, cb) {
		...
		var DRPTemplate = '<div class="daterangepicker dropdown-menu">' + '<div class="calendar left"></div>' + '<div class="calendar right"></div>'
				+ '<div class="ranges">' + '<div class="range_inputs">' + '<div class="daterangepicker_start_input" style="float: left">'
				...
				+ '</button>&nbsp;'
				+ '<button class="'
				+ this.clearClass + ' clearBtn">' + this.locale.clearLabel + '</button>' + '</div>' + '</div>' + '</div>';
		...
		if (hasOptions) {
			...
			if (typeof options.ranges == 'object') {
				for ( var range in options.ranges) {
					...
					// If we have a min/max date set, bound this range to it,
					// but only if it would otherwise fall outside of the min/max.
	...
	DateRangePicker.prototype = {
		...
		clickDate : function(e) {
			...
			if (cal.hasClass('left')) {
				...
				if (typeof this.dateLimit == 'object') {
					...
					var minDate = new moment(endDate).add(negConfig);
		...
		buildCalendar : function(month, year) {
			...
			var firstDay = moment().startOf('day').day(1).month(month).year(year);
			var lastMonth = firstDay.clone().subtract('days', 1).month();
			var lastYear = firstDay.clone().subtract('days', 1).year();
			...
			var curDate = moment().startOf('day').day(startDay).month(lastMonth).year(lastYear);
			for ( var i = 0, col = 0, row = 0; i < 42; i++, col++, curDate.add('day', 1)) {
				if (i > 0 && col % 7 === 0) {
					col = 0;
					row++;
				}
		...
		renderCalendar : function(calendar, selected, minDate, maxDate) {
			...
			for ( var row = 0; row < 6; row++) {
				...
				for ( var col = 0; col < 7; col++) {
					...
					html += '<td class="' + cname.replace(/\s+/g, ' ').replace(/^\s?(.*?)\s?$/, '$1') + '" title="' + title + '">' + calendar[row][col].date()
							+ '</td>';
				}
				html += '</tr>';
			}
